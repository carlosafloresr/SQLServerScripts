/*
EXECUTE USP_DriverHistory 'DNJ', 'D0008'
*/
CREATE PROCEDURE USP_DriverHistory
		@Company	Varchar(5), 
...
BEGIN
	...
	ELSE
		SET @Query	= 'SELECT Code AS VendorId, Div_Code AS Division, Cmpy_No AS CompanyId FROM trk.driver WHERE Cmpy_No = ' + CAST(@CompanyNo AS Varchar) + ' AND Div_Code IN (''34'',''36'')'
END
ELSE
BEGIN
	IF @WithAgents = 1
...
SELECT	DISTINCT VM.VendorId
		...
		,VM.Company
		,CAST(ISNULL(VO.HireDate,VM.HireDate) AS Date) AS HireDate
		,VM.DriverType
		,VM.MyTruckStartDate
		,VM.TerminationDate
		,VM.SubType
...
IF @DriverId IS Null
	SET @MinDate = (SELECT HireDate FROM #TempVendorMaster)
...
SELECT	Company
		...
		,DriverType
		,HireDate
		,CAST(WeekEndDate + 5 AS Date) AS WeekEndDate
		,Drayage
		--,Miles
		,DriverFuelRebate
		,FuelAmount
		...
		--,CAST(CASE WHEN Gallons = 0 THEN 0 ELSE FuelAmount/Gallons * 1.00 END AS Numeric(10,2)) AS GrossPPG
		--,CAST(CASE WHEN Gallons = 0 THEN 0 ELSE (FuelAmount - DriverFuelRebate)/Gallons * 1.00 END AS Numeric(10,2)) AS NetPPG
		--,CAST(Drayage + DriverFuelRebate - FuelAmount AS Numeric(10,2)) AS NetPayAfterFuel
		...
		,Savings
		,TagsandTaxes
		,CAST(ManualATP AS Numeric(10,2)) AS ManualATP
		,CAST(TiresDeduction AS Numeric(10,2)) AS TiresDeduction
		,OtherInsurance
		,OtherDeductions + EscrowInterest AS OtherDeductions
		,CashAdvance
		,CheckAmount
		--,MyTruckBalance
		--,CAST(ISNULL(SafetyBonus, 0) AS Numeric(10,2)) AS SafetyBonus
		--,RowNumber
		--,ContractualEscrowBalance = CAST((SELECT SUM(Amount) FROM GPCustom.DBO.View_EscrowTransactions ES WHERE ES.CompanyId = @Company AND Fk_EscrowModuleId = 1 AND VendorId = TP.VendorId AND PostingDate <= TP.WeekEndDate + 5 AND DeletedBy IS Null) AS Numeric(10,2))
--INTO	#tmpResults
FROM	#TempData TP
ORDER BY VendorId, WeekEndDate

--INSERT INTO OPENROWSET ('Microsoft.Jet.OLEDB.4.0', 'Excel 8.0;Database=C:\TEMP\Test.xls;HDR=YES','SELECT * FROM #tmpResults') SELECT * FROM #tmpResults
