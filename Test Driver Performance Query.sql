DECLARE	@Company	Varchar(5) = 'DNJ', 
		@OnlyDetail Bit = 1, 
		@DriverType Int = 0, 
		@RunDate	Datetime = '05/25/2016',
		@DriverId	Varchar(12) = Null, --'A1325',
...
BEGIN
	SELECT	Company
			...
			,RowNumber
			,ContractualEscrowBalance = (SELECT SUM(Amount) FROM GPCustom.DBO.View_EscrowTransactions ES WHERE ES.CompanyId = @Company AND Fk_EscrowModuleId = 1 AND VendorId = TP.VendorId AND PostingDate <= TP.WeekEndDate + 5 AND DeletedBy IS Null)
	FROM	#TempData TP
END
