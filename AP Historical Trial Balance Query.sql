...
SELECT  W.VENDORID ,
        ...
        W.AGINGBUCKET ,
        W.DOCUMENTAMT ,
		CASE WHEN DATEDIFF(d, W.DOCDATE, @RunDate) <= 30 THEN W.CURTRXAMTT
			ELSE 0 END [Current],
		CASE WHEN DATEDIFF(d, W.DOCDATE, @RunDate) <= 30 THEN W.CURTRXAMTT
			ELSE 0 END [0_to_30_Days],
		CASE WHEN DATEDIFF(d, W.DOCDATE, @RunDate) BETWEEN 31 AND 60 THEN W.CURTRXAMTT
			ELSE 0 END [31_to_60_Days],
		CASE WHEN DATEDIFF(d, W.DOCDATE, @RunDate) BETWEEN 61 AND 90 THEN W.CURTRXAMTT
			ELSE 0 END [61_to_90_Days],
		CASE WHEN DATEDIFF(d, W.DOCDATE, @RunDate) BETWEEN 91 AND 180 THEN W.CURTRXAMTT
			ELSE 0 END [91_to_180_Days],
		CASE WHEN DATEDIFF(d, W.DOCDATE, @RunDate) > 180 THEN W.CURTRXAMTT
