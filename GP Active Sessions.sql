...
DECLARE	@Counter			Int,
		@InactivityTime		Int = 100 --(SELECT VarI FROM GPCustom.dbo.Parameters WHERE ParameterCode = 'GPINACTIVEMINUTES')
...
BEGIN
	...
	WHERE	SQLSESID IN (SELECT Spid FROM @tblGPUsers WHERE InactiveMinutes > @InactivityTime)
