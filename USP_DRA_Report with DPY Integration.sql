/*
EXECUTE AIS.dbo.USP_DRA_Report 'AIS','DSDR051916DD', 'A0376'

EXECUTE USP_DRA_Report 'OIS','DSDR041416DD'
...
EXECUTE USP_DRA_Report 'IMC','042816DSDRVDD','13443'
...
SET		@WeekEndDate	= GPCustom.dbo.DayFwdBack(@WEndDate, 'P', 'Saturday')

PRINT 'Weekend Date: ' + CAST(@WEndDate AS Varchar)
...
PRINT '3: Find drayage records ' + CONVERT(Varchar, GETDATE(), 109)

SELECT	DriverId,
		Drayage + DriverFuelRebate AS Drayage,
		GPCustom.dbo.DayFwdBack(WeekEndDate, 'N', 'Thursday') AS WeekEndDate
INTO	#tmpDrayage
FROM	GPCustom.dbo.View_Integration_DPY_ByDriverId
WHERE	Company = @Company 
		AND WeekEndDate BETWEEN DATEADD(dd, -120, @WeekEndDate) AND @WeekEndDate

PRINT '4: Find pay week records ' + CONVERT(Varchar, GETDATE(), 109)
				...
				FROM	(
						SELECT	DISTINCT @CpnyName AS Company,
								...
								CASE	WHEN (LEFT(ISNULL(A2.BACHNUMB, AP.BACHNUMB), 3) = 'DPY' OR LEFT(ISNULL(A2.BACHNUMB, AP.BACHNUMB), 2) = 'DP') THEN 'Drayage'
										...
										WHEN LEFT(ISNULL(A2.BACHNUMB, AP.BACHNUMB), 3) = 'OOS' THEN (SELECT TOP 1 Description FROM GPCustom.dbo.OOS_DeductionTypes WHERE Company = PH.Company AND DeductionCode = SUBSTRING(PD.DocNumbr, 1, PATINDEX('%' + LTRIM(RTRIM(PH.VendorId)) + '%', PD.DocNumbr) - 1))
								ELSE PD.DocNumbr END AS DeductionType,
								CASE	WHEN (LEFT(ISNULL(A2.BACHNUMB, AP.BACHNUMB), 3) = 'DPY' OR LEFT(ISNULL(A2.BACHNUMB, AP.BACHNUMB), 2) = 'DP') THEN DY.Drayage ELSE PD.Outstanding_Amount END AS DeductionAmount,
								PH.ChekTotl,
								PD.ApFvchNm,
						...
						FROM	GPCustom.dbo.PM10300 PH
								...
								LEFT JOIN #tmpDrayage DY ON PH.VendorId = DY.DriverId AND CASE	WHEN GPCustom.dbo.WeekDay(PD.DocDate) < 5 THEN PD.DocDate + (5 - GPCustom.dbo.WeekDay(PD.DocDate))
										WHEN GPCustom.dbo.WeekDay(PD.DocDate) = 5 THEN PD.DocDate
										WHEN LEFT(PD.VchrNmbr, 3) = 'FPT' THEN PD.DocDate + (12 - GPCustom.dbo.WeekDay(PD.DocDate))
										ELSE PD.DocDate + (12 - GPCustom.dbo.WeekDay(PD.DocDate)) END = DY.WeekEndDate
						...
						SELECT	@CpnyName AS Company,
								...
								CASE	WHEN (LEFT(TB.BACHNUMB, 3) = 'DPY' OR LEFT(TB.BACHNUMB, 2) = 'DP') THEN 'Drayage'
										...
										WHEN LEFT(TB.BACHNUMB, 3) = 'OOS' THEN (SELECT TOP 1 Description FROM GPCustom.dbo.OOS_DeductionTypes WHERE Company = @Company AND DeductionCode = LEFT(TB.DocNumbr, GPCustom.dbo.AT(RTRIM(TB.VendorId), TB.DocNumbr, 1) - 1))
...
PRINT '5: Main query selection ' + CONVERT(Varchar, GETDATE(), 109)
...
PRINT '6: Final presentation query ' + CONVERT(Varchar, GETDATE(), 109)

DROP TABLE #tmpDrayage	
