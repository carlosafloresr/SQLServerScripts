/*
EXECUTE GPCustom.dbo.USP_Intercompany_Report_Blackline_New 1, '2022-12'
*/
ALTER PROCEDURE USP_Intercompany_Report_Blackline_New
		@JustTest		Bit = 0,
		@RunPeriod		Char(7) = Null
AS
SET NOCOUNT ON
...
DECLARE @tblPeriod		Table (Year1 Int, PeriodId Char(2))

IF @RunPeriod IS Null
BEGIN
	SET @Query = N'SELECT TOP 1 YEAR1, PERIODID
	...
	SELECT @Year = Year1, @Period = PeriodId FROM @tblPeriod
END
ELSE
BEGIN
	SET @Year	= CAST(LEFT(@RunPeriod, 4) AS Int)
	SET @Period = CAST(RIGHT(@RunPeriod, 2) AS Int)
END
...
BEGIN
	...
	WHERE	GL2.OPENYEAR = ' + CAST(@Year AS Varchar) + '
			AND GL2.PERIODID = ' + CAST(@Period AS Varchar) + '
			AND GL2.VOIDED = 0 
			AND GL5.ACTNUMST = ''' + @Account + ''' 
	UNION
	SELECT ''' + @Company + ''',''' + dbo.PADL(@CpyNumber, 2, '0') + ''', ''' + @Intercompany + ''', GL5.ACTNUMST, ''' + @Description + ''',
			''' + @AcctDesc + ''',
			GL2.PERIODID,
			GL2.JRNENTRY,
			GL2.TRXSORCE,
			...
			GL2.CRDTAMNT,
			GL2.DEBITAMT - GL2.CRDTAMNT
	FROM	' + @Company + '.dbo.GL30000 GL2
			INNER JOIN ' + @Company + '.dbo.GL00105 GL5 ON GL2.ACTINDX = GL5.ACTINDX
	WHERE	GL2.HSTYEAR = ' + CAST(@Year AS Varchar) + '
			AND GL2.PERIODID = ' + CAST(@Period AS Varchar) + '
			AND GL2.VOIDED = 0 
			AND GL5.ACTNUMST = ''' + @Account + ''' 
	ORDER BY
...
IF @JustTest = 1
BEGIN
	DELETE tmpIntercompany

	INSERT INTO tmpIntercompany
