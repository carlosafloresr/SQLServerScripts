/*
EXECUTE USP_AP_Historical_Trial_Balance_Report 'AIS', '10/29/2022', 'A51702', 0, Null, 1, 1
EXECUTE USP_AP_Historical_Trial_Balance_Report 'AIS', '10/29/2022', 'ALL', 1, '', 1, 1
...
SET @Query = N'SELECT	pmTrans.vendorid, 
		...
		CASE WHEN DATEDIFF(dd, DOCDATE, ''' + CONVERT(Char(10), @CutoffDate, 101) + ''') BETWEEN 91 AND 180 THEN pmTrans.docamnt - ISNULL(apply.appldamt, 0) ELSE 0 END AS [91_to_180_Days], 
        CASE WHEN DATEDIFF(dd, DOCDATE, ''' + CONVERT(Char(10), @CutoffDate, 101) + ''') > 180 THEN pmTrans.docamnt - ISNULL(apply.appldamt, 0) ELSE 0 END AS [181_and_Over]
...
BEGIN
	IF @ForReporting = 0
		...
		BEGIN
			...
			FROM	@tblTrialData DAT
			GROUP BY VENDORID, VENDNAME, VNDCLASS
			HAVING	SUM([0_to_30_Days] + [31_to_60_Days] + [61_to_90_Days] + [91_to_180_Days] + [181_and_Over]) <> 0
			UNION
			SELECT	@CompanyName AS CompanyName,
					'ZZZZZZZZ' AS VENDORID,
					'ZZZZZZZZ' AS VENDNAME,
