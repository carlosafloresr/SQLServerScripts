...
EXECUTE USP_DRA_Report 'AIS','DSDR102011CK','A0695'
...
FROM	(
		...
		FROM	(
				...
				--		(ABS(DOCAMNT) - ApplyTo) * CASE WHEN TB.DocType = 5 THEN -1 ELSE 1 END AS DeductionAmount,
				--		NULL,
				--		TB.DocNumbr,
				...
				--				PH.PYMTRMID, 
				--				PH.VOIDED,
				--				ApplyTo = 0.00 --(SELECT ISNULL(SUM(PM20100.APPLDAMT), 0.00) FROM PM20100 WHERE PM20100.APTODCNM = PH.DOCNUMBR AND PM20100.VENDORID = PH.VENDORID AND PM20100.ApplyToGLPostDate < @CutoffDate)
				--		FROM	PM20000 PH
				--				INNER JOIN #VednorMaster VM ON PH.VendorId = VM.VendorId
				--				LEFT JOIN PM20100 AP ON PH.VendorId = AP.VendorId AND PH.DocNumbr = AP.ApToDcNm
				--		WHERE	PH.POSTEDDT <= @CutoffDate AND VM.EFT = @IsEFT
				--		UNION
				--		SELECT	PH.VCHRNMBR,
				--				PH.VENDORID,
				--				PH.DOCTYPE,
				...
				--				PH.PYMTRMID, 
				--				PH.VOIDED,
				--				ApplyTo = (SELECT ISNULL(SUM(PM30300.APPLDAMT), 0.00) FROM PM30300 WHERE PM30300.APTODCNM = PH.DOCNUMBR AND PM30300.VENDORID = PH.VENDORID AND PM30300.GLPOSTDT < @CutoffDate - 6)
				--		FROM	PM30200 PH
				--				INNER JOIN #VednorMaster VM ON PH.VendorId = VM.VendorId
				--				LEFT JOIN PM30300 AP ON PH.VendorId = AP.VendorId AND PH.DocNumbr = AP.ApToDcNm
				--		WHERE	POSTEDDT BETWEEN @CutoffDate - 6 AND @CutoffDate AND VM.EFT = @IsEFT
				--		) TB --> SubQuery
				...
				--		--AND TB.VendorId NOT IN (SELECT VendorId FROM GPCustom.dbo.PM10300 PH
				--		--					WHERE	PH.Company = @Company AND
				--		--							PH.DOCDATE BETWEEN @WEndDate - 13 AND @WEndDate - 6 AND
				--		--							PH.ChekTotl > 0 AND
				--		--							(@DriverId IS Null OR (@DriverId IS NOT Null AND PH.VendorId = @DriverId))
				--		--						)
				--		) AL
				--WHERE	(@DriverId IS Null OR (@DriverId IS NOT Null AND AL.VendorId = @DriverId))
				UNION	-- Historical Transactions
				SELECT	@CpnyName AS Company,
						CASE	WHEN GPCustom.dbo.WeekDay(TB.DocDate) < 5 THEN TB.DocDate + (5 - GPCustom.dbo.WeekDay(TB.DocDate))
								WHEN GPCustom.dbo.WeekDay(TB.DocDate) = 5 THEN TB.DocDate
								WHEN LEFT(TB.VchrNmbr, 3) = 'FPT' THEN TB.DocDate + (12 - GPCustom.dbo.WeekDay(TB.DocDate))
